...
func TestGetAllJobs(t *testing.T) {
	...
	require.Equal(t, testJobRequest.Job.Name, response.Jobs[0].Name)
	require.Equal(t, tCtx.GetNamespaceName(), response.Jobs[0].Namespace)
}
			...
			Input: &api.CreateRayJobRequest{
				Job: &api.RayJob{
					...
					ClusterSelector: map[string]string{
						"ray.io/cluster": tCtx.GetRayClusterName(),
					},
					JobSubmitter: &api.RayJobSubmitter{
						Image:  cluster.ClusterSpec.HeadGroupSpec.Image,
						Cpu:    "0.1",
