...
def test_cluster():
    ...
    head = HeadNodeSpec(compute_template="default-template",
                        ...
                        image="rayproject/ray:2.8.1-py310", service_type=ServiceType.ClusterIP,
    ...
    worker = WorkerNodeSpec(group_name="small", compute_template="default-template", replicas=1,
                            ...
                            image="rayproject/ray:2.8.1-py310", volumes=[volume], environment=environment)
    cluster = Cluster(name="test", namespace="default", user="boris", version="2.8.1",
...
def test_job_submission():
    ...
    worker = WorkerNodeSpec(group_name="small", compute_template="default-template", replicas=1,
                            min_replicas=1, max_replicas=1, ray_start_params=DEFAULT_WORKER_START_PARAMS,
                            image="rayproject/ray:2.8.1-py310", volumes=[volume], environment=environment)
    cluster = Cluster(name="test-job", namespace="default", user="boris", version="2.8.1",
                      cluster_spec=ClusterSpec(head_node=head, worker_groups=[worker]))
    # create
    status, error = apis.create_cluster(cluster)
    assert status == 200
