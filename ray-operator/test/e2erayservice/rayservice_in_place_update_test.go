...
func TestRayServiceInPlaceUpdateWithMaximumName(t *testing.T) {
	test := With(t)
	g := NewWithT(t)

	// Create a namespace
	namespace := test.NewTestNamespace()
	rayServiceName := strings.Repeat("s", utils.MaxRayServiceNameLength)
	...
	_, err = test.Client().Core().CoreV1().Services(namespace.Name).Get(test.Ctx(), rayService.Name+"-head-svc", metav1.GetOptions{})
	g.Expect(err).NotTo(HaveOccurred())
	_, err = test.Client().Core().CoreV1().Services(namespace.Name).Get(test.Ctx(), rayService.Name+"-serve-svc", metav1.GetOptions{})
	g.Expect(err).NotTo(HaveOccurred())
