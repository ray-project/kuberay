...
func TestRayClusterAutoscalerIdleTimeout(t *testing.T) {
	tc := tests[1] // V2 autoscaler test case only
	t.Run(tc.name, func(t *testing.T) {
		test := With(t)
		g := gomega.NewWithT(t)
		...
		// RayCluster with mixed idleTimeoutSeconds values
		rayClusterSpecAC := rayv1ac.RayClusterSpec().
			WithEnableInTreeAutoscaling(true).
			WithRayVersion(GetRayVersion()).
			WithAutoscalerOptions(rayv1ac.AutoscalerOptions().
				WithIdleTimeoutSeconds(60). // Global default
			...
			WithWorkerGroupSpecs(
				rayv1ac.WorkerGroupSpec().
					...
					WithMinReplicas(0).
					WithMaxReplicas(1).
					WithIdleTimeoutSeconds(30). // Faster than global
					WithGroupName("fast-idle-group").
					WithRayStartParams(map[string]string{"num-cpus": "1"}).
					WithTemplate(tc.WorkerPodTemplateGetter()),
				rayv1ac.WorkerGroupSpec().
					WithReplicas(0).
					WithMinReplicas(0).
					WithMaxReplicas(1).
					WithIdleTimeoutSeconds(0). // Zero idle timeout
					WithGroupName("zero-idle-group").
					WithRayStartParams(map[string]string{"num-cpus": "1"}).
					WithTemplate(tc.WorkerPodTemplateGetter()),
				rayv1ac.WorkerGroupSpec().
					WithReplicas(0).
					WithMinReplicas(0).
					WithMaxReplicas(1).
					// No IdleTimeoutSeconds - uses global default of 60
					WithGroupName("default-idle-group").
					WithRayStartParams(map[string]string{"num-cpus": "1"}).
					WithTemplate(tc.WorkerPodTemplateGetter()),
			)
		...
		// Wait for RayCluster to become ready and verify the configuration is accepted
		g.Eventually(RayCluster(test, rayCluster.Namespace, rayCluster.Name), TestTimeoutMedium).
			Should(gomega.WithTransform(RayClusterState, gomega.Equal(rayv1.Ready)))
		LogWithTimestamp(test.T(), "RayCluster %s/%s is ready", rayCluster.Namespace, rayCluster.Name)

		// Test validation - create a RayCluster with invalid negative idleTimeoutSeconds (should fail)
		invalidRayClusterSpecAC := rayv1ac.RayClusterSpec().
			WithEnableInTreeAutoscaling(true).
			WithRayVersion(GetRayVersion()).
			WithAutoscalerOptions(rayv1ac.AutoscalerOptions().
			...
			WithWorkerGroupSpecs(
				rayv1ac.WorkerGroupSpec().
					...
					WithMinReplicas(0).
					WithMaxReplicas(1).
					WithIdleTimeoutSeconds(-10). // Invalid negative value
					WithGroupName("invalid-group").
					WithRayStartParams(map[string]string{"num-cpus": "1"}).
					WithTemplate(tc.WorkerPodTemplateGetter()),
			)

		invalidRayClusterAC := rayv1ac.RayCluster("invalid-ray-cluster", namespace.Name).
			WithSpec(invalidRayClusterSpecAC)

		_, err = test.Client().Ray().RayV1().RayClusters(namespace.Name).Apply(test.Ctx(), invalidRayClusterAC, TestApplyOptions)
		g.Expect(err).To(gomega.HaveOccurred())
		LogWithTimestamp(test.T(), "Successfully rejected RayCluster with negative idleTimeoutSeconds: %v", err)
	})
}
