...
func updateWorkerPodsToRunningAndReady(ctx context.Context, rayClusterName string, namespace string) {
	...
	workerFilterLabels := client.MatchingLabels{
		utils.RayClusterLabelKey:   rayClusterName,
	...
	gomega.Eventually(
		...
		time.Second*15, time.Millisecond*500).Should(gomega.Equal(int(numWorkerPods)), "workerGroup: %v", workerPods.Items)
	...
	gomega.Eventually(
		...
		time.Second*15, time.Millisecond*500).Should(gomega.BeTrue(), "Worker Pods should be running: %v", workerPods.Items)
}
