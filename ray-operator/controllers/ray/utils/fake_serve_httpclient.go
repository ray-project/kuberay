...
type FakeRayDashboardClient struct {
	multiAppStatuses map[string]*utilstype.ServeApplicationStatus
	GetJobInfoMock   atomic.Pointer[func(context.Context, string) (*utilstype.RayJobInfo, error)]
	BaseDashboardClient
	serveDetails utilstype.ServeDetails
}
...
func (r *FakeRayDashboardClient) GetMultiApplicationStatus(_ context.Context) (map[string]*utilstype.ServeApplicationStatus, error) {
	return r.multiAppStatuses, nil
}

func (r *FakeRayDashboardClient) GetServeDetails(_ context.Context) (*utilstype.ServeDetails, error) {
	return &r.serveDetails, nil
}

func (r *FakeRayDashboardClient) SetMultiApplicationStatuses(statuses map[string]*utilstype.ServeApplicationStatus) {
	r.multiAppStatuses = statuses
}

func (r *FakeRayDashboardClient) GetJobInfo(ctx context.Context, jobId string) (*utilstype.RayJobInfo, error) {
	if mock := r.GetJobInfoMock.Load(); mock != nil {
		return (*mock)(ctx, jobId)
	}
	return &utilstype.RayJobInfo{JobStatus: rayv1.JobStatusRunning}, nil
}

func (r *FakeRayDashboardClient) ListJobs(ctx context.Context) (*[]utilstype.RayJobInfo, error) {
	if mock := r.GetJobInfoMock.Load(); mock != nil {
		info, err := (*mock)(ctx, "job_id")
		if err != nil {
			return nil, err
		}
		return &[]utilstype.RayJobInfo{*info}, nil
	}
	return nil, nil
}
...
func (r *FakeRayDashboardClient) SubmitJobReq(_ context.Context, _ *utilstype.RayJobRequest, _ *string) (string, error) {
	return "", nil
}
