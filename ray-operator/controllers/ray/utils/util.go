...
// DeleteMTLSAutoGeneratedSecrets removes operator-managed TLS secrets created in auto-generate mode.
// BYOC secrets are user-owned and must not be deleted by the operator.
func DeleteMTLSAutoGeneratedSecrets(ctx context.Context, c client.Client, cluster *rayv1.RayCluster) error {
	if IsTLSBYOC(&cluster.Spec) {
		return nil
	}
	secrets := []string{
		GetCASecretName(cluster.Name, cluster.UID),
		fmt.Sprintf("%s-%s", RayHeadSecretPrefix, cluster.Name),
		fmt.Sprintf("%s-%s", RayWorkerSecretPrefix, cluster.Name),
	}
...
// IsMTLSControllerActive reports whether the dedicated RayClusterMTLS controller is registered.
// It requires both the feature gate and cert-manager to be available at operator startup.
func IsMTLSControllerActive(certManagerAvailable bool) bool {
	return features.Enabled(features.RayClusterMTLS) && certManagerAvailable
}
