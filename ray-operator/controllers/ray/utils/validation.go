...
func validateDeletionConfiguration(rayJob *rayv1.RayJob) error {
	...
	// Hard-fail if DeletionStrategy is set but deletionRules is empty.
	if len(rayJob.Spec.DeletionStrategy.DeletionRules) == 0 {
		return fmt.Errorf("the RayJob spec is invalid: DeletionStrategy.deletionRules field must be specified and non-empty")
	}
