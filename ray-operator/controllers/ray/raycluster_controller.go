...
func (r *RayClusterReconciler) reconcilePods(ctx context.Context, instance *rayv1.RayCluster) error {
	...
	for _, worker := range instance.Spec.WorkerGroupSpecs {
		...
		multihostIndexingEnabled := features.Enabled(features.RayMulithostIndexing) && worker.NumOfHosts > 1 && !utils.IsAutoscalingV2Enabled(&instance.Spec)
		// Map current existing pods by label
		workerGrpReplicaMap := make(map[string][]corev1.Pod)
		if multihostIndexingEnabled {
			for _, pod := range workerPods.Items {
				if value, ok := pod.GetLabels()["ray.io/replica-index"]; ok {
		...
		if diff > 0 {
			...
			if multihostIndexingEnabled {
				// This case should not occur as pods will be deleted in replica groups
