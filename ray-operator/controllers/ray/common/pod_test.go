...
func TestBuildPod(t *testing.T) {
	...
	pod := BuildPod(podTemplateSpec, rayiov1alpha1.HeadNode, cluster.Spec.HeadGroupSpec.RayStartParams, svcName, "6379", nil, "", cluster.Spec.HeadGroupSpec.Template.Spec.Containers[0].Env)
	...
	pod = BuildPod(podTemplateSpec, rayiov1alpha1.WorkerNode, worker.RayStartParams, svcName, "6379", nil, "", cluster.Spec.HeadGroupSpec.Template.Spec.Containers[0].Env)
...
func TestBuildPod_WithAutoscalerEnabled(t *testing.T) {
	...
	svcName := utils.GenerateServiceName(cluster.Name)
	podTemplateSpec := DefaultHeadPodTemplate(*cluster, cluster.Spec.HeadGroupSpec, podName, svcName, "6379")
	pod := BuildPod(podTemplateSpec, rayiov1alpha1.HeadNode, cluster.Spec.HeadGroupSpec.RayStartParams, svcName, "6379", &trueFlag, "", cluster.Spec.HeadGroupSpec.Template.Spec.Containers[0].Env)
...
func TestBuildPod_WithCreatedByRayService(t *testing.T) {
	...
	svcName := utils.GenerateServiceName(cluster.Name)
	podTemplateSpec := DefaultHeadPodTemplate(*cluster, cluster.Spec.HeadGroupSpec, podName, svcName, "6379")
	pod := BuildPod(podTemplateSpec, rayiov1alpha1.HeadNode, cluster.Spec.HeadGroupSpec.RayStartParams, svcName, "6379", &trueFlag, RayServiceCreatorLabelValue, cluster.Spec.HeadGroupSpec.Template.Spec.Containers[0].Env)
