...
func setContainerEnvVars(pod *corev1.Pod, rayNodeType rayv1.RayNodeType, fqdnRayIP string, headPort string, rayStartCmd string, creatorCRDType utils.CRDType, defaultContainerEnvs []corev1.EnvVar) {
	...
	//
	// The command is commonly invoked as `$KUBERAY_GEN_RAY_START_CMD` by users
	// who overwrite the Ray container command. Shell parameter expansion is not
	// recursive: variable references and control operators produced by expanding
	// KUBERAY_GEN_RAY_START_CMD would otherwise remain literal arguments. Prefix
	// the generated value with `eval` so the shell parses the operator-generated
	// command once more and resolves runtime values such as KUBERAY_POD_IP.
	generatedRayStartCmdEnv := corev1.EnvVar{
		Name:  utils.KUBERAY_GEN_RAY_START_CMD,
		Value: "eval " + rayStartCmd,
	}
