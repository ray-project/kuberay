	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			listOptions := &client.ListOptions{
				LabelSelector: labels.SelectorFromSet(map[string]string{utils.RayClusterHeadlessServiceLabelKey: cluster.Name}),
				Namespace:     cluster.Namespace,
			}
			...
			serviceList := corev1.ServiceList{}
			require.NoError(t, fakeClient.List(ctx, &serviceList, listOptions))
			if !tc.expectService {
			...
			assert.Equal(t, expectedName, serviceList.Items[0].Name)
			assert.Equal(t, "None", serviceList.Items[0].Spec.ClusterIP, "Created service is not a headless service")
