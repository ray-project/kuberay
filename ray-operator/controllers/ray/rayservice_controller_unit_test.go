...
// TestDeleteRayServiceOwnedResources verifies that deleteRayServiceOwnedResources
// removes RayClusters and Services labeled with the RayService association, plus
// the named Gateway and HTTPRoute. Per-cluster Serve services with cluster-scoped
// labels are intentionally not listed here — they are owner-ref'd to the
// RayCluster and rely on Kubernetes garbage collection.
func TestDeleteRayServiceOwnedResources(t *testing.T) {
	// Gateway/HTTPRoute deletion is gated on RayServiceIncrementalUpgrade; the
	// test exercises that path too, so we enable it.
	features.SetFeatureGateDuringTest(t, features.RayServiceIncrementalUpgrade, true)

	ctx := context.TODO()
	namespace := "test-ns"
	rsName := "test-rayservice"

	newScheme := runtime.NewScheme()
	require.NoError(t, rayv1.AddToScheme(newScheme))
	require.NoError(t, corev1.AddToScheme(newScheme))
	require.NoError(t, gwv1.Install(newScheme))

	rayService := &rayv1.RayService{
		ObjectMeta: metav1.ObjectMeta{Name: rsName, Namespace: namespace},
	}
	ownedLabels := map[string]string{
		utils.RayOriginatedFromCRNameLabelKey: rsName,
		utils.RayOriginatedFromCRDLabelKey:    utils.RayOriginatedFromCRDLabelValue(utils.RayServiceCRD),
	}
	unrelatedLabels := map[string]string{
		utils.RayOriginatedFromCRNameLabelKey: "another-rayservice",
		utils.RayOriginatedFromCRDLabelKey:    utils.RayOriginatedFromCRDLabelValue(utils.RayServiceCRD),
	}

	cluster := &rayv1.RayCluster{ObjectMeta: metav1.ObjectMeta{Name: "owned-cluster", Namespace: namespace, Labels: ownedLabels}}
	unrelatedCluster := &rayv1.RayCluster{ObjectMeta: metav1.ObjectMeta{Name: "other-cluster", Namespace: namespace, Labels: unrelatedLabels}}
	headSvc := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: "head-svc", Namespace: namespace, Labels: ownedLabels}}
	serveSvc := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: "serve-svc", Namespace: namespace, Labels: ownedLabels}}
	unrelatedSvc := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: "other-svc", Namespace: namespace, Labels: unrelatedLabels}}
	gatewayName := common.RayServiceGatewayNamespacedName(rayService)
	gw := &gwv1.Gateway{ObjectMeta: metav1.ObjectMeta{Name: gatewayName.Name, Namespace: gatewayName.Namespace}}
	httpRouteName := common.RayServiceHTTPRouteNamespacedName(rayService)
	route := &gwv1.HTTPRoute{ObjectMeta: metav1.ObjectMeta{Name: httpRouteName.Name, Namespace: httpRouteName.Namespace}}

	fakeClient := clientFake.NewClientBuilder().
		WithScheme(newScheme).
		WithRuntimeObjects(rayService, cluster, unrelatedCluster, headSvc, serveSvc, unrelatedSvc, gw, route).
		Build()

	reconciler := &RayServiceReconciler{Client: fakeClient, Scheme: newScheme, Recorder: record.NewFakeRecorder(10)}

	allDeleted, err := reconciler.deleteRayServiceOwnedResources(ctx, rayService)
	require.NoError(t, err)
	// At least one resource was present, so this pass cannot have completed.
	assert.False(t, allDeleted)

	// Owned resources should be gone.
	assert.True(t, errors.IsNotFound(fakeClient.Get(ctx, client.ObjectKeyFromObject(cluster), &rayv1.RayCluster{})))
	assert.True(t, errors.IsNotFound(fakeClient.Get(ctx, client.ObjectKeyFromObject(headSvc), &corev1.Service{})))
	assert.True(t, errors.IsNotFound(fakeClient.Get(ctx, client.ObjectKeyFromObject(serveSvc), &corev1.Service{})))
	assert.True(t, errors.IsNotFound(fakeClient.Get(ctx, gatewayName, &gwv1.Gateway{})))
	assert.True(t, errors.IsNotFound(fakeClient.Get(ctx, httpRouteName, &gwv1.HTTPRoute{})))

	// Unrelated resources should remain.
	require.NoError(t, fakeClient.Get(ctx, client.ObjectKeyFromObject(unrelatedCluster), &rayv1.RayCluster{}))
	require.NoError(t, fakeClient.Get(ctx, client.ObjectKeyFromObject(unrelatedSvc), &corev1.Service{}))

	// Re-run: nothing left to delete → allDeleted=true.
	allDeleted, err = reconciler.deleteRayServiceOwnedResources(ctx, rayService)
	require.NoError(t, err)
	assert.True(t, allDeleted)
}

// TestHandleSuspendStateMachine verifies handleSuspend transitions across the
// (no suspend) → Suspending (committed, no deletion yet) → Suspending (deleting)
// → Suspended → (no suspend) lifecycle.
func TestHandleSuspendStateMachine(t *testing.T) {
	ctx := context.TODO()
	namespace := "test-ns"
	rsName := "test-rayservice"
	...
	rayService := &rayv1.RayService{
		...
		Status: rayv1.RayServiceStatuses{
			...
			ActiveServiceStatus:  rayv1.RayServiceStatus{RayClusterName: "owned-cluster"},
			PendingServiceStatus: rayv1.RayServiceStatus{RayClusterName: "pending-cluster"},
			NumServeEndpoints:    3,
		},
	}
	cluster := &rayv1.RayCluster{ObjectMeta: metav1.ObjectMeta{Name: "owned-cluster", Namespace: namespace, Labels: ownedLabels}}
	...
	fakeClient := clientFake.NewClientBuilder().
		WithScheme(newScheme).
		WithRuntimeObjects(rayService, cluster).
		WithStatusSubresource(rayService).
		Build()

	reconciler := &RayServiceReconciler{Client: fakeClient, Scheme: newScheme, Recorder: record.NewFakeRecorder(10)}

	// runPhase simulates one full reconcile of handleSuspend + the caller's
	// status persistence step.
	runPhase := func(t *testing.T) (*rayv1.RayService, bool, ctrl.Result) {
		t.Helper()
		working := &rayv1.RayService{}
		require.NoError(t, fakeClient.Get(ctx, client.ObjectKeyFromObject(rayService), working))
		// Preserve Spec.Suspend across phases by reapplying from the test's source object.
		working.Spec = rayService.Spec
		handled, result, err := reconciler.handleSuspend(ctx, working)
		require.NoError(t, err)
		// Caller persists status changes — this is the single status-update site.
		require.NoError(t, fakeClient.Status().Update(ctx, working))
		return working, handled, result
	}

	// Phase 1: commit-only pass. Suspending condition is persisted alongside
	// the status reset, but no resource is deleted yet.
	working, handled, result := runPhase(t)
	require.True(t, handled)
	assert.NotZero(t, result.RequeueAfter)
	assert.True(t, meta.IsStatusConditionTrue(working.Status.Conditions, string(rayv1.RayServiceSuspending)))
	assert.False(t, meta.IsStatusConditionTrue(working.Status.Conditions, string(rayv1.RayServiceSuspended)))
	assert.Empty(t, working.Status.ActiveServiceStatus.RayClusterName)
	assert.Empty(t, working.Status.PendingServiceStatus.RayClusterName)
	assert.EqualValues(t, 0, working.Status.NumServeEndpoints)
	assert.Equal(t, rayv1.NotRunning, working.Status.ServiceStatus)
	// Cluster is intentionally NOT deleted in phase 1.
	require.NoError(t, fakeClient.Get(ctx, client.ObjectKeyFromObject(cluster), &rayv1.RayCluster{}))

	// Phase 2: Suspending is committed; this pass triggers the actual delete.
	// allDeleted=false because the cluster was present at the start of the
	// pass — matches real K8s where deletion is eventual — so Suspending
	// stays True and we requeue.
	working, handled, result = runPhase(t)
	require.True(t, handled)
	assert.NotZero(t, result.RequeueAfter)
	assert.True(t, meta.IsStatusConditionTrue(working.Status.Conditions, string(rayv1.RayServiceSuspending)))
	assert.False(t, meta.IsStatusConditionTrue(working.Status.Conditions, string(rayv1.RayServiceSuspended)))
	require.True(t, errors.IsNotFound(fakeClient.Get(ctx, client.ObjectKeyFromObject(cluster), &rayv1.RayCluster{})))

	// Phase 3: nothing left to delete on this pass → transitions to Suspended.
	working, handled, _ = runPhase(t)
	require.True(t, handled)
	assert.False(t, meta.IsStatusConditionTrue(working.Status.Conditions, string(rayv1.RayServiceSuspending)))
	assert.True(t, meta.IsStatusConditionTrue(working.Status.Conditions, string(rayv1.RayServiceSuspended)))
	assert.Equal(t, metav1.ConditionFalse,
		meta.FindStatusCondition(working.Status.Conditions, string(rayv1.RayServiceReady)).Status)

	// Phase 4: Spec.Suspend flipped back to false → exits Suspended, returns
	// handled=false so the main reconcile can recreate resources.
	rayService.Spec.Suspend = false
	working, handled, _ = runPhase(t)
	assert.False(t, handled)
	assert.Nil(t, meta.FindStatusCondition(working.Status.Conditions, string(rayv1.RayServiceSuspended)))
}

// TestHandleSuspendAtomic verifies that once the Suspending condition is set,
// deletion completes even if Spec.Suspend is flipped back to false mid-way.
func TestHandleSuspendAtomic(t *testing.T) {
	ctx := context.TODO()
	namespace := "test-ns"
	rsName := "test-rayservice"
	...
	// RayService already mid-suspend; Spec.Suspend has been flipped back to false.
	rayService := &rayv1.RayService{
		ObjectMeta: metav1.ObjectMeta{Name: rsName, Namespace: namespace},
		Spec:       rayv1.RayServiceSpec{Suspend: false},
		Status: rayv1.RayServiceStatuses{
			Conditions: []metav1.Condition{{
				Type:   string(rayv1.RayServiceSuspending),
				Status: metav1.ConditionTrue,
				Reason: string(rayv1.SuspendRequested),
			}},
		},
	}
	cluster := &rayv1.RayCluster{ObjectMeta: metav1.ObjectMeta{Name: "owned-cluster", Namespace: namespace, Labels: ownedLabels}}
	...
	working := rayService.DeepCopy()
	handled, _, err := reconciler.handleSuspend(ctx, working)
	require.NoError(t, err)
	// Even with Spec.Suspend=false, an in-progress suspend continues deleting.
	require.True(t, handled)
	require.True(t, errors.IsNotFound(fakeClient.Get(ctx, client.ObjectKeyFromObject(cluster), &rayv1.RayCluster{})))
}
