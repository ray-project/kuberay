...
func TestIsSubmitterContainerFinished(t *testing.T) {
	t.Run("Submitter terminated", func(t *testing.T) {
		pod := &corev1.Pod{
		...
		assert.True(t, isSubmitterContainerFinished(pod))
	})

	t.Run("Submitter running but ray-head is completed", func(t *testing.T) {
		pod := &corev1.Pod{
			Status: corev1.PodStatus{
				ContainerStatuses: []corev1.ContainerStatus{
					{
		...
		assert.False(t, isSubmitterContainerFinished(pod))
	})

	t.Run("Other container terminated but not submitter -> false", func(t *testing.T) {
		pod := &corev1.Pod{
			Status: corev1.PodStatus{
				ContainerStatuses: []corev1.ContainerStatus{
					{
...
func TestConfigureSubmitterContainer(t *testing.T) {
	t.Run("K8sJobMode: empty command -> set default bash and args; env populated", func(t *testing.T) {
		...
		rayJob := &rayv1.RayJob{
			Spec: rayv1.RayJobSpec{
				...
				EntrypointResources: `{"Custom_1": 1}`,
		...
		err := configureSubmitterContainer(&container, rayJob, rayv1.K8sJobMode)
		require.NoError(t, err)

		// command should be default bash with -e option
		assert.Equal(t, utils.GetContainerCommand([]string{"e"}), container.Command)

		expectedCmd, err := common.BuildJobSubmitCommand(rayJob, rayv1.K8sJobMode)
		require.NoError(t, err)
		assert.Equal(t, []string{strings.Join(expectedCmd, " ")}, container.Args)

		assert.Contains(t, container.Env, corev1.EnvVar{Name: PythonUnbufferedEnvVarName, Value: "1"})
		assert.Contains(t, container.Env, corev1.EnvVar{Name: utils.RAY_DASHBOARD_ADDRESS, Value: rayJob.Status.DashboardURL})
		assert.Contains(t, container.Env, corev1.EnvVar{Name: utils.RAY_JOB_SUBMISSION_ID, Value: rayJob.Status.JobId})
	})

	t.Run("K8sJobMode: preset command -> do not override; env populated", func(t *testing.T) {
		container := corev1.Container{
			Command: []string{"/bin/sh", "-c", "--"},
			Args:    []string{"preset command"},
		}
		rayJob := &rayv1.RayJob{
			Spec: rayv1.RayJobSpec{
				RayClusterSpec: &rayv1.RayClusterSpec{RayVersion: "2.6.0"},
		...
		assert.Equal(t, []string{"preset command"}, container.Args)
	...
	t.Run("SidecarMode: preset command -> always override; env populated", func(t *testing.T) {
		container := corev1.Container{
			Command: []string{"/bin/sh", "-c", "--"},
			Args:    []string{"preset command"},
		}
		rayJob := &rayv1.RayJob{
			Spec: rayv1.RayJobSpec{
				...
				Entrypoint: "echo sidecar",
			},
			Status: rayv1.RayJobStatus{
				DashboardURL: "head-svc.ns.svc.cluster.local:8265",
				JobId:        "job-789",
			},
		}

		err := configureSubmitterContainer(&container, rayJob, rayv1.SidecarMode)
		require.NoError(t, err)

		// command should be overridden to default bash with -e
		assert.Equal(t, utils.GetContainerCommand([]string{"e"}), container.Command)

		// args should be joined BuildJobSubmitCommand (SidecarMode)
		expectedCmd, err := common.BuildJobSubmitCommand(rayJob, rayv1.SidecarMode)
		require.NoError(t, err)
		assert.Equal(t, []string{strings.Join(expectedCmd, " ")}, container.Args)
