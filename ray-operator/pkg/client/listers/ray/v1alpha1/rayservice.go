...
type RayServiceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.RayService, err error)
...
// List lists all RayServices in the indexer.
func (s *rayServiceLister) List(selector labels.Selector) (ret []*v1alpha1.RayService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.RayService))
	})
	return ret, err
}
...
type RayServiceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.RayService, error)
...
// List lists all RayServices in the indexer for a given namespace.
func (s rayServiceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.RayService, err error) {
...
// Get retrieves the RayService from the indexer for a given namespace and name.
func (s rayServiceNamespaceLister) Get(name string) (*v1alpha1.RayService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("rayservice"), name)
	}
	return obj.(*v1alpha1.RayService), nil
}
