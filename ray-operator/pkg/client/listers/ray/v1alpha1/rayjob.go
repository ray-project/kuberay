...
type RayJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.RayJob, err error)
...
// List lists all RayJobs in the indexer.
func (s *rayJobLister) List(selector labels.Selector) (ret []*v1alpha1.RayJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.RayJob))
	})
	return ret, err
}
...
type RayJobNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.RayJob, error)
...
// List lists all RayJobs in the indexer for a given namespace.
func (s rayJobNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.RayJob, err error) {
...
// Get retrieves the RayJob from the indexer for a given namespace and name.
func (s rayJobNamespaceLister) Get(name string) (*v1alpha1.RayJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("rayjob"), name)
	}
	return obj.(*v1alpha1.RayJob), nil
}
