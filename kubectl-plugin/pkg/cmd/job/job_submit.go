...
func (options *SubmitJobOptions) Run(ctx context.Context, factory cmdutil.Factory) error {
	...
	if options.fileName == "" {
		...
		rayJobApplyConfigResult, err := k8sClients.RayClient().RayV1().RayJobs(options.namespace).Apply(ctx, rayJobApplyConfig, v1.ApplyOptions{FieldManager: "kubectl-plugin"})
		if err != nil {
		...
		options.RayJob, err = k8sClients.RayClient().RayV1().RayJobs(options.namespace).Create(ctx, options.RayJob, v1.CreateOptions{})
		if err != nil {
	...
	_, err = k8sClients.RayClient().RayV1().RayJobs(options.namespace).Update(ctx, options.RayJob, v1.UpdateOptions{})
	if err != nil {
