...
func TestRayScaleClusterComplete(t *testing.T) {
	tests := []struct {
		...
		{
			name:              "namespace and worker group should be set correctly",
			args:              []string{"my-workergroup"},
			namespace:         "DEADBEEF",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := fakeScaleClusterOptions.Complete([]string{"my-workergroup"})

			require.NoError(t, err)
			assert.Equal(t, tc.expectedNamespace, *fakeScaleClusterOptions.configFlags.Namespace)
			assert.Equal(t, "my-workergroup", fakeScaleClusterOptions.workerGroup)
		})
	}
}
...
func TestRayScaleClusterValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectError: "must specify -c/--ray-cluster",
		},
		{
