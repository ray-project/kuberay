...
When a `RayCluster` enables in-tree autoscaling (`spec.enableInTreeAutoscaling: true`), the
operator creates and reconciles a dedicated `ServiceAccount`, `Role`, and `RoleBinding` in the
RayCluster namespace. The operator's own `ServiceAccount` must be allowed to manage these
resources.

If you provide custom RBAC or restrict the rules generated by this chart, retain the following
permissions for the operator in every namespace it watches:
...
generated objects by name. The operator's default controller-runtime client may use informers for
these reads. If an informer cannot list one of these resources, autoscaler reconciliation can
remain blocked instead of returning the underlying authorization error, and other `RayCluster`
reconciliations may be delayed when the controller is busy.

For a restricted installation, grant equivalent namespaced permissions in each namespace listed by
`watchNamespace`. If `rbacEnable` is `false`, create the equivalent rules yourself before enabling
in-tree autoscaling. You can check the operator's effective permissions with:
