...
// must process events (possibly empty) and persist a snapshot.
func TestProcessSession_DeadNoSnapshot_Processes(t *testing.T) {
	...
	status, err := p.processSession(context.Background(), session)
	if err != nil {
	...
	// Sanity: subsequent call should be a no-op (skip-if-exists).
	status, err = p.processSession(context.Background(), session)
	if err != nil {
...
// is skipped cleanly: no events read, no snapshot written.
...
// as "dead"). The caller logs the error and retries on the next tick.
