...
func (r *RayLogHandler) processSessionLatestLogs() {
	...
	if r.IsHead {
		metafile := r.getMetafilePath(sessionID)
...
// getMetafilePath returns the metafile path for a given session ID
func (r *RayLogHandler) getMetafilePath(sessionID string) string {
	metadir := path.Join(r.RootDir, utils.METADIR_NAME)
	var metafile string
	if r.OwnerKind == "" {
		dirName := fmt.Sprintf("%s_%s", r.RayClusterNamespace, r.RayClusterName)
		metafile = path.Clean(path.Join(
			metadir,
			dirName,
			path.Base(sessionID)))
	} else {
		dirName := fmt.Sprintf("%s_%s_%s_%s", strings.ToLower(r.OwnerKind), r.OwnerName, r.RayClusterNamespace, r.RayClusterName)
		metafile = path.Clean(path.Join(
			metadir,
			dirName,
			path.Base(sessionID)))
