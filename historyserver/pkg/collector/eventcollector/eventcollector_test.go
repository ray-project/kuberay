...
func TestPersistEvents_Buffered_BuffersAndFlushes(t *testing.T) {
	...
	assert.Regexp(t, `node-1-2026-05-13-10-\d+$`, nodeKey, "unexpected node key: %s", nodeKey)
	assert.Regexp(t, `node-1-2026-05-13-10-\d+$`, jobKey, "unexpected job key: %s", jobKey)
...
func TestFlushMemEvents_AggregatesByHourBucket(t *testing.T) {
	...
	var h10, h11 string
	h10Re := regexp.MustCompile(`-2026-05-13-10-\d+$`)
	h11Re := regexp.MustCompile(`-2026-05-13-11-\d+$`)
	for _, k := range keys {
		if h10Re.MatchString(k) {
			h10 = k
		} else if h11Re.MatchString(k) {
			h11 = k
		}
	}
...
// with a UnixNano suffix appended to the file name.
func TestBuildLegacyEventStorageKey(t *testing.T) {
	ec := newTestCollector(t, newMockWriter(), Options{CompressionEnabled: false})

	ts := time.Date(2026, 5, 13, 10, 30, 0, 123456789, time.UTC)
	nano := ts.UnixNano()

	nodeKey := ec.buildLegacyEventStorageKey(categoryNodeEvents, "2026-05-13-10", "node-1", "session_abc", ts)
	assert.Equal(t, fmt.Sprintf("history/cluster_ns/session_abc/node_events/node-1-2026-05-13-10-%d", nano), nodeKey)

	jobKey := ec.buildLegacyEventStorageKey(path.Join(categoryJobPrefix, "job-7"), "2026-05-13-10", "node-1", "session_abc", ts)
	assert.Equal(t, fmt.Sprintf("history/cluster_ns/session_abc/job_events/job-7/node-1-2026-05-13-10-%d", nano), jobKey)
}
