...
func TestListFiles(t *testing.T) {
	initialObjects := []fakestorage.Object{
		{
			ObjectAttrs: fakestorage.ObjectAttrs{
				...
				Name:       "ray_historyserver/cluster1_ns1/logs/file1.txt",
			},
			Content: []byte("a"),
		},
		{
			ObjectAttrs: fakestorage.ObjectAttrs{
				BucketName: "test-bucket",
				Name:       "ray_historyserver/cluster1_ns1/logs/file2.log",
			},
			Content: []byte("b"),
		},
		{
			ObjectAttrs: fakestorage.ObjectAttrs{
				BucketName: "test-bucket",
				Name:       "ray_historyserver/cluster1_ns1/logs/subdir/",
			},
			Content: []byte(""),
		},
		{
			ObjectAttrs: fakestorage.ObjectAttrs{
				BucketName: "test-bucket",
				Name:       "ray_historyserver/cluster1_ns1/logs/subdir/file3.txt",
			},
			Content: []byte("c"),
		},
		{
			ObjectAttrs: fakestorage.ObjectAttrs{
				BucketName: "test-bucket",
				Name:       "ray_historyserver/cluster1_ns1/other/file4.txt",
			},
			Content: []byte("d"),
		},
		{
			ObjectAttrs: fakestorage.ObjectAttrs{
				BucketName: "test-bucket",
				Name:       "ray_historyserver/cluster2_ns2/logs/file5.txt",
			},
			Content: []byte("e"),
		},
		{
			ObjectAttrs: fakestorage.ObjectAttrs{
				BucketName: "test-bucket",
				Name:       "ray_historyserver/cluster2_ns2/logs/subdir2/",
			},
			Content: []byte("e"),
		},
	}
	...
	tests := []struct {
		name                 string
		clusterNameNamespace string
		directory            string
		expected             []string
	}{
		{
			name:                 "list_files",
			clusterNameNamespace: "cluster1_ns1",
		...
		{
			name:                 "list_cluster2",
			clusterNameNamespace: "cluster2_ns2",
			directory:            utils.RAY_SESSIONDIR_LOGDIR_NAME,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			files := handler.ListFiles(tc.clusterNameNamespace, tc.directory)
			sort.Strings(files)
			sort.Strings(tc.expected)
			if diff := cmp.Diff(tc.expected, files); diff != "" {
				t.Errorf("ListFiles(%q, %q) returned diff (-want +got):\n%s", tc.clusterNameNamespace, tc.directory, diff)
			}
		})
	}
}
...
func TestGetContent(t *testing.T) {
	clusterNameNamespace := "clusterA_default"
	fileName := "important.log"
	objPath := "ray_historyserver/clusterA_default/session123/logs/" + fileName
	...
	reader := handler.GetContent(clusterNameNamespace, fileName)
	if reader == nil {
		t.Fatalf("GetContent(%q, %q) returned nil reader, expected non-nil", clusterNameNamespace, fileName)
	}
	...
	if string(content) != fileContent {
		t.Errorf("GetContent(%q, %q) content mismatch: got %q, want %q", clusterNameNamespace, fileName, string(content), fileContent)
	}
}
