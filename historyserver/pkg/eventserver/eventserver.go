...
// StoreEvent is a public wrapper around the private storeEvent. It is exposed
// so that v2 beta's event processor can drive event storage directly from a
// synchronous per-session loader (ProcessSingleSession). v1 code paths continue
// to use storeEvent unchanged.
func (h *EventHandler) StoreEvent(eventMap map[string]any) error {
	return h.storeEvent(eventMap)
}
