...
// SessionActorMap uses sessionName as the key to store actors for each session.
type SessionActorMap struct {
	SessionActorMap map[string]*ActorMap
	Mu              sync.RWMutex
}

func NewSessionActorMap() *SessionActorMap {
	return &SessionActorMap{
		SessionActorMap: make(map[string]*ActorMap),
	}
}

func (s *SessionActorMap) RLock() {
	s.Mu.RLock()
}

func (s *SessionActorMap) RUnlock() {
	s.Mu.RUnlock()
}

func (s *SessionActorMap) Lock() {
	s.Mu.Lock()
}

func (s *SessionActorMap) Unlock() {
	s.Mu.Unlock()
}

// GetOrCreateActorMap returns the ActorMap for the given session, creating it if it doesn't exist.
func (s *SessionActorMap) GetOrCreateActorMap(sessionName string) *ActorMap {
	s.Lock()
	defer s.Unlock()

	actorMap, exists := s.SessionActorMap[sessionName]
	if !exists {
		actorMap = NewActorMap()
		s.SessionActorMap[sessionName] = actorMap
	}
	return actorMap
}

// ClusterSessionActorMap provides session-scoped actor storage.
// Uses secondary index to get the actor map.
type ClusterSessionActorMap struct {
	ClusterSessionActorMap map[string]*SessionActorMap
	Mu                     sync.RWMutex
}
...
func (c *ClusterSessionActorMap) RUnlock() {
	c.Mu.RUnlock()
}

func (c *ClusterSessionActorMap) Lock() {
	c.Mu.Lock()
}

func (c *ClusterSessionActorMap) Unlock() {
	c.Mu.Unlock()
}

// GetOrCreateSessionActorMap returns the SessionActorMap for the given cluster, creating it if it doesn't exist.
func (c *ClusterSessionActorMap) GetOrCreateSessionActorMap(clusterName string) *SessionActorMap {
	c.Lock()
	defer c.Unlock()

	sessionActorMap, exists := c.ClusterSessionActorMap[clusterName]
	if !exists {
		sessionActorMap = NewSessionActorMap()
		c.ClusterSessionActorMap[clusterName] = sessionActorMap
	}
	return sessionActorMap
}
