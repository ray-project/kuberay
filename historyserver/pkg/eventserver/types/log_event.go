...
// job_id, in their raw (non-camelCased) form. It is additive to v1 — the
// existing GetAllEvents/GetEventsByJobID APIs return API-response maps suited
// to the HTTP handlers, whereas the v2 beta snapshot pipeline needs the
// original struct representation so it can be re-serialized as JSON and later
// rehydrated into typed objects.
//
// Events are sorted by timestamp ascending per job, matching the order that
// GetAllEvents produces. If the session is unknown, an empty (non-nil) map
// is returned so callers can treat absence uniformly.
