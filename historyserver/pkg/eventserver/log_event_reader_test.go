...
func (m *logEventMockReader) addFile(clusterNameNamespace, filePath, content string) {
	if m.files[clusterNameNamespace] == nil {
		m.files[clusterNameNamespace] = make(map[string]string)
	}
	m.files[clusterNameNamespace][filePath] = content
}

func (m *logEventMockReader) addDir(clusterNameNamespace, dirPath string, entries []string) {
	if m.dirs[clusterNameNamespace] == nil {
		m.dirs[clusterNameNamespace] = make(map[string][]string)
	}
	m.dirs[clusterNameNamespace][dirPath] = entries
}

func (m *logEventMockReader) List() []utils.ClusterInfo { return nil }

func (m *logEventMockReader) GetContent(clusterNameNamespace string, fileName string) io.Reader {
	if cd, ok := m.files[clusterNameNamespace]; ok {
...
func (m *logEventMockReader) ListFiles(clusterNameNamespace string, dir string) []string {
	if cd, ok := m.dirs[clusterNameNamespace]; ok {
