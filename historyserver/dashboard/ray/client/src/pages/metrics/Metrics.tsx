              ...
              >
                ...
                {dashboardUids?.["data"] && (
                  <Tooltip title="The Ray Data dashboard has a dropdown to filter the data metrics by Dataset ID">
                    <MenuItem
                      component="a"
                      href={`${grafanaHost}/d/${dashboardUids["data"]}/?`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      Ray Data Dashboard
          ...
          <Box sx={{ margin: 1 }}>
            {METRICS_CONFIG.map((config) => (
              <MetricsSection
                ...
                dashboardParams={grafanaDefaultDashboardUidParams}
              />
            ))}
            {dashboardUids?.["data"] &&
              DATA_METRICS_CONFIG.map((config) => (
                <MetricsSection
                  ...
                  dashboardDatasource={grafanaDefaultDatasource}
                  dashboardParams={dashboardUids["data_params"]}
                />
              ))}
          </Box>
        </div>
    ...
    >
      ...
      >
        {contents.map(({ title, pathParams }) => {
          const path =
            `/d-solo/${dashboardUid}?${dashboardParams}&${pathParams}&${refreshParams}${timeRangeParams}&var-SessionName=${sessionName}`;
          // const path =
