...
When the head Pod was deleted, it will be recreated with a new IP，and the GCS server address is changed accordingly too. The Raylets of all workers will try to get GCS address from Redis in ‘ReconnectGcsServer’, but the redis_client always use the previous head IP, so it will always fail to get new GCS address. The Raylet will not exit until max retries are reached. There are two configurations determines this delay:
