...
if [ "$IS_FROM_RAY_RELEASE_AUTOMATION" = 1 ]; then
    ...
    KUBERAY_TEST_RAY_IMAGE="rayproject/ray:nightly.$(date +'%y%m%d').${RAY_NIGHTLY_COMMIT:0:6}-py310" && export KUBERAY_TEST_RAY_IMAGE
else
    ...
    RAY_NIGHTLY_COMMIT="a5d3e3c80ec604e4242dd5f5899d0851969aba0a"
    KUBERAY_TEST_RAY_IMAGE="rayproject/ray:nightly.$(date +'%y%m%d').${RAY_NIGHTLY_COMMIT:0:6}-py310" && export KUBERAY_TEST_RAY_IMAGE
fi
